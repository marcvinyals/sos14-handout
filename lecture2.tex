...
\begin{definition}
  ...
  \begin{alignat*}{2}
    \minimize  b^{T}y\\
    \subjectto  y_{1}A_{1}+\cdots+y_{m}A_{m} -C \succeq 0
  \end{alignat*}
\end{definition}
...
\begin{theorem}
  ...
  the primal and dual respectively. Moreover, if the primal has a
  strictly feasible solution (a solution $X$ such that $X \succ 0$,
  \ie $X$ is positive definite) then 
  \begin{enumerate}
    \item The dual optimum is attained (which is not always the case
    for SDPs)
    \item $ v_{P} = v_{D}$ 
    \item 
  \end{enumerate}
